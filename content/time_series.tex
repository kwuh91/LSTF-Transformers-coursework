...
Нередко на практике встречаются частные случаи данной модели, например \text{MA}(1) и 
\text{MA}(2):
\begin{align*}
...
называют авторегрессионным процессом порядка $p$, или кратко, \text{AR}($p$).

Аналогично MA, нередко на практике встречаются частные случаи данной модели, например \text{AR}(1) и 
\text{AR}(2):
\begin{align*}
...
% ARCH, GARCH, ARIMAX, подбор параметров + пример для ARIMA? 
...
% стр. 7: возможно убрать фразу: `Можно заметить сходство с корреляцией Пирсона:`, 
% тк аквтокорреляция по сути и есть корреляция Пирсона
