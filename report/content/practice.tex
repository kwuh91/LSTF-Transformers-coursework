...
% replace embedding layer in informer with two conv1d layers (+ batchNorm and GELU)
...
\subsubsection{Датасеты}

В данной работе мы проводим эксперименты на 
n датасетах.

\paragraph{ETT}

\textbf{ETT} (Electricity Transformer Temperature) \cite{informer}: 
...
\paragraph{Training} % Optimiser/LR schedule, batch size, AMP
\paragraph{Baselines} % compare with everything
\paragraph{Hyper-parameter tuning} % check in informer article
\paragraph{Setup} % check in informer article 
\paragraph{Metrics} % MSE, RMSE, MAE? ...
\paragraph{Platform} % my gpu
